      ...
      try {
        if (category === "srchBooks") {
          const response = await axios.get(
            `http://data4library.kr/api/${category}?authKey=[API키]&title=${srchValue}&exactMatch=true&pageNo=1&pageSize=10&format=json`
          );
          for (let i = 0; i <= 9; i++) {
            array.push({
              doc: {
        ...
        if (category === "loanItemSrch") {
          const response = await axios.get(
            `http://data4library.kr/api/${category}?authKey=[API키]&age=${selectedAge}&pageNo=1&pageSize=10&format=json`
          );
